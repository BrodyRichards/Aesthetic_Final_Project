...
SubShader {
	Pass {
		...
		half4 frag (v2f i) : COLOR
		{
			...
			half4 bluecolor = tex2D(_MainTex, (i.uv.xy + direction.x - direction.y)*0.01 * filterRadius * _ColorIntensity);

			color += fixed4(redcolor.r, redcolor.b, redcolor.g, 1) *  step(filterRadius, -0.001);
			color *= 1 - 0.5 * step(filterRadius, -0.001);

			color += fixed4(greencolor.g, greencolor.b, greencolor.r, 1) *  step(0.005, filterRadius);
			color *= 1 - 0.5 * step(0.005, filterRadius);
